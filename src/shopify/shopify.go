...
func Verify(api *Api, r *http.Request) error {
	received, message, err := parseHmacAndMessage(r)		
	if(err != nil) {
		return err
	}
	computed := hmacSHA256(message, api.Secret)
...
func OAuthUrl(api *Api, host, shop, state string) string {
